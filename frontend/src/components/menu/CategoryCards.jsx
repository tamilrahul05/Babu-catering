    ...
    <Link to={`/menu/${cat.id}`} className="w-full sm:w-[320px] md:w-[350px] group/link">
      <div className="relative aspect-[3/4] rounded-[2.5rem] overflow-hidden">
        <motion.div 
          ...
          viewport={{ once: true }}
          whileHover={{ y: -10 }}
          className="w-full h-full relative rounded-[2.5rem] overflow-hidden transition-all ring-1 ring-white/10 group-hover/link:ring-amber-500/30 shadow-2xl bg-zinc-900"
        >
          {!isLoaded && (
          ...
          />
          <div className="absolute inset-0 bg-gradient-to-t from-black via-black/30 to-transparent opacity-80 group-hover/link:opacity-90 transition-opacity"></div>
          
          <div className="absolute inset-x-6 bottom-6 p-6 md:p-8 bg-zinc-950/40 backdrop-blur-xl border border-white/10 rounded-[2rem] text-white transition-all duration-500 group-hover/link:bg-zinc-950/60 group-hover/link:border-amber-500/30">
