...
const Showcase = ({ showPreview, setShowPreview, categoryId, selectedItems, handleBooking }) => {
 return (
 <AnimatePresence>
 {showPreview && (
 <motion.div
 ...
 animate={{ opacity: 1 }}
 exit={{ opacity: 0 }}
 className="fixed inset-0 z-[100] flex flex-col items-center justify-center p-4 sm:p-6 bg-zinc-950/95 overflow-hidden h-screen max-h-[100vh]"
 >
 ...
 <div className="text-xl md:text-3xl font-black text-green-500 tracking-tighter">
 ₹{selectedItems.length * 50} <span className="text-[10px] md:text-xs text-zinc-500 font-bold uppercase ml-1">/ plate</span>
 </div>
 </div>

 <div className="flex items-center gap-2 sm:hidden">
 <button className="p-2.5 bg-zinc-800 text-green-500 rounded-xl border border-zinc-700" aria-label="Share">
 <Share2 size={18} />
 </button>
 <button className="p-2.5 bg-zinc-800 text-green-500 rounded-xl border border-zinc-700" aria-label="Download">
 <Download size={18} />
 </button>
 </div>
 </div>
 ...
 >
 <Edit2 size={14} className="text-green-500" /> Adjust Items
 </button>
 <button className="flex items-center justify-center w-11 h-11 bg-zinc-800 hover:bg-zinc-700 text-green-500 rounded-full transition-all border border-zinc-700">
 <Share2 size={18} />
 </button>
 <button className="flex items-center justify-center w-11 h-11 bg-zinc-800 hover:bg-zinc-700 text-green-500 rounded-full transition-all border border-zinc-700">
 <Download size={18} />
 ...
 className="w-full sm:w-auto bg-gradient-to-r from-green-400 to-green-600 hover:from-green-500 hover:to-green-700 text-black px-8 md:px-10 py-4 md:py-4 rounded-2xl md:rounded-full font-black text-base md:text-lg flex items-center justify-center gap-3 transition-all duration-300 shadow-xl shadow-green-500/20 active:scale-95"
 >
